...
export const sessions: MiddlewareHandler = async (ctx, next) => {
  ...
  const session = sessionId ? await actionStore.get(sessionId, { type: "json" }) : undefined;

  if (session && sessionId) {
    setActionResult(session.actionName, session.actionResult);
    // Optional: delete the session after the page is rendered.
    // Feel free to implement your own persistence strategys
