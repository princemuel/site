...
export const ratelimit: MiddlewareHandler = async (ctx, next) => {
  if (!(ctx.request.url.includes("/api/") || ctx.request.url.includes("_actions"))) return next();
  ...
  try {
    const ip = import.meta.env.DEV
      ? "anonymous"
      : (ctx.request.headers.get("x-forwarded-for") ?? ctx.request.headers.get("x-real-ip"));
