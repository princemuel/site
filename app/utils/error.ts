...
/**
 * Strip the internal library frames from an error's stack trace so
 * consumers see only the frames relevant to *their* code.
 */
...
/**
 * Matches printf-style placeholders: %s %d %i %j %o
 * Captures an optional leading "%" so "%%s" can be treated as an
 * escaped, literal "%s" rather than a placeholder.
 * The `u` flag makes `.` and character handling Unicode-aware, so
 * messages containing astral-plane characters (emoji, rare CJK, etc.)
 * are scanned correctly instead of splitting surrogate pairs.
 */
